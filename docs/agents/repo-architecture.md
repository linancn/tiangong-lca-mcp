...
lastReviewedNote: 'Reviewed for Issue #62 and PR #63: the image path emits one ARM64 manifest, reuses scan-on-push or starts a missing scan, and fails closed unless ECR is COMPLETE with exactly zero HIGH/CRITICAL.'
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
  - ./repo-validation.md
...
The build upgrades Alpine packages before package-manager setup and records the installed OpenSSL version. The ECR qualification command disables provenance so the tag resolves to one scan-compatible ARM64 image manifest instead of an OCI index. The tag is commit-bearing and must be absent before push. Qualification reuses scan-on-push or starts a scan only when none exists, then exits before run unless the result is COMPLETE with exactly zero CRITICAL and HIGH findings. A vulnerable or unscannable image remains evidence only and is never an ECS task input.
