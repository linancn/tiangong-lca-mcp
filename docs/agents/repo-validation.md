...
lastReviewedNote: 'Reviewed for Issue #62 and PR #63: Docker validation requires a no-provenance single ARM64 manifest and a tested fail-closed assertion before run unless ECR is COMPLETE with zero HIGH/CRITICAL.'
related:
  - ../../AGENTS.md
  - ../../.docpact/config.yaml
...
| package, pnpm, Node, TypeScript, lint, Docker, or client config | `pnpm prepush:gate` | inspect `pnpm list --depth Infinity` when compiler or runtime leakage is in scope; after publishing, compare registry version/integrity and packed broker files, then run a no-cache no-provenance Linux ARM64 Docker build, inspect patched OS packages plus OCI architecture/PATH/default bin, push an absent immutable-style tag, verify a single scan-compatible manifest, and wait for a COMPLETE zero-HIGH/CRITICAL ECR scan | Require `apk upgrade --no-cache` before Corepack, `--provenance=false` on the ECR qualification build, and a tested fail-closed COMPLETE/zero-CRITICAL/zero-HIGH assertion before `docker run`; Docker's global package pin must equal `package.json.version`, Corepack must be enabled before exact activation/use, and `/pnpm/bin` must remain on OCI PATH. Packed-consumer proof must contain auth middleware, OAuth broker store/runtime, Supabase broker, and HTTP app. Recheck `DEV_EN.md` and `DEV_CN.md` whenever the exact baseline or startup path changes. |
